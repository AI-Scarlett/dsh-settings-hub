...
- Immutable public source Commit: unavailable until the `0.2.1` source release is published and read back.
- General audit: rerun against the `0.2.1` source before publishing.
- Marketplace candidate: intentionally deferred until the `0.2.1` source Commit exists; never pin an uncommitted future SHA.
...
The prepared source does not rewrite the Registry or prove refreshed marketplace visibility or Profile installation. A fixed-source candidate may be generated only after the public source Commit exists.
