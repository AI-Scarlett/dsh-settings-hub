...
test('Client registers additive section, plugin tab and shell overlay on public rc.2 slots', async () => {
  ...
  assert.equal(textOf(fixture.tree).split('设置快捷方式').length - 1, 1)
...
test('theme-aware typography keeps current DSH tokens in both color schemes', async () => {
  ...
  const intro = findNode(fixture.tree, node => node.type === 'p' && textOf(node).startsWith('配置 Settings Hub 自有入口'))
  const scopeNotice = findNode(fixture.tree, node => node.props?.['aria-label'] === '设置生效范围')
  ...
  assert.match(textOf(scopeNotice), /官方侧边栏.*第三方 Tab/)
...
test('icon selection previews immediately and persists with the bounded v2 preference shape', async () => {
  ...
  tree = fixture.render()
  assert.match(textOf(tree), /Settings Hub 自有首页、内页、自有 Tab 与右下角面板已同步/)
